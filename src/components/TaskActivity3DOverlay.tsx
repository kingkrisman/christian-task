...
const SuccessScene: React.FC = () => {
  return (
    <>
      ...
      <Sparkles count={80} scale={[4, 2, 2]} size={4} speed={1.2} color="#78D700" />
...
const LoadingScene: React.FC = () => {
  return (
    <>
      ...
      <SpinnerMesh />
      <Text position={[0, -1.2, 0]} fontSize={0.28} color="#E5E7EB" anchorX="center" anchorY="middle">
        Adding task…
      </Text>
      <Environment preset="city" />
    </>
...
export const TaskActivity3DOverlay: React.FC<OverlayProps> = ({ mode, isDark = false, onDone }) => {
  useEffect(() => {
  ...
  return (
    <div className={`absolute inset-0 z-50 flex items-center justify-center ${isDark ? "bg-black/50" : "bg-white/50"} backdrop-blur-xs`}> 
      <div className={`w-72 h-48 rounded-xl shadow-card border ${isDark ? "bg-dark-card border-dark-border" : "bg-white border-brand-gray-200"}`}>
